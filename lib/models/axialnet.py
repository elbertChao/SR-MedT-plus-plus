from .memory_efficient_attention import MemoryEfficientAxialAttention
import math
import torch
...
class SpatialAttention(nn.Module):
    """Lightweight spatial attention: pools channels then learns a single-channel spatial weight map."""
...
class AxialAttention(MemoryEfficientAxialAttention):
    """Memory efficient version of Axial Attention"""
    def __init__(self, in_planes, out_planes, groups=8, kernel_size=56,
                 stride=1, bias=False, width=False):
        super(AxialAttention, self).__init__(in_planes, out_planes, groups, kernel_size,
                                           stride, bias, width)
...
class AxialAttention_dynamic(nn.Module):
    def __init__(self, in_planes, out_planes, groups=8, kernel_size=56,
        ...
        self.f_sv  = nn.Parameter(torch.tensor(0.1), requires_grad=True)
    ...
    def forward(self, x):
        ...
        # clamp prevents learnable gates from drifting to 0 or inf, which caused NaN in later folds
        qr = torch.mul(qr, self.f_qr.clamp(0.001, 1.0))
        kr = torch.mul(kr, self.f_kr.clamp(0.001, 1.0))
        ...
        sv  = torch.mul(sv,  self.f_sv.clamp(0.001, 1.0))
        sve = torch.mul(sve, self.f_sve.clamp(0.001, 1.0))
...
class medt_net(nn.Module):
    ...
    def __init__(self, block, block_2, layers, num_classes=2, zero_init_residual=True,
        ...
        mid_channels = int(128 * s)

        self.conv1 = nn.Conv2d(imgchan, self.inplanes, kernel_size=7, stride=2, padding=3,
                               bias=False)
        self.conv2 = nn.Conv2d(self.inplanes, mid_channels, kernel_size=3, stride=1, padding=1, bias=False)
        self.conv3 = nn.Conv2d(mid_channels, self.inplanes, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1 = norm_layer(self.inplanes)
        ...
        # reflect padding reduces edge artifacts in the local branch
        self.conv1_p = nn.Conv2d(imgchan, self.inplanes, kernel_size=7, stride=2, padding=3,
                               bias=False, padding_mode='reflect')
        self.conv2_p = nn.Conv2d(self.inplanes, mid_channels, kernel_size=3, stride=1, padding=1,
                               bias=False, padding_mode='reflect')
        self.conv3_p = nn.Conv2d(mid_channels, self.inplanes, kernel_size=3, stride=1, padding=1,
                               bias=False, padding_mode='reflect')
        self.bn1_p = norm_layer(self.inplanes)
        self.bn2_p = norm_layer(mid_channels)
        ...
        # GroupNorm instead of BN: more stable than BN at the small effective batch sizes used here
        self.fusion_block = nn.Sequential(
            nn.Conv2d(int(128*s) * 2, int(128*s), kernel_size=3, padding=1, bias=False),
            nn.GroupNorm(num_groups=min(int(128*s), 8), num_channels=int(128*s)),
            nn.ReLU(inplace=True)
        )

        local_ch = int(128 * s)
        self.cross_patch_comm = nn.Sequential(
            nn.Conv2d(local_ch, local_ch, kernel_size=3, padding=1,
                    groups=local_ch, bias=False),
            nn.GroupNorm(num_groups=min(local_ch, 8), num_channels=local_ch),
            nn.ReLU(inplace=True),
            nn.Conv2d(local_ch, local_ch, kernel_size=1, bias=False),
            nn.GroupNorm(num_groups=min(local_ch, 8), num_channels=local_ch),
            nn.ReLU(inplace=True)
        )
        ...
        self.drop_l_enc4 = nn.Dropout2d(self.dropout_rate)
        self.drop_l_dec2 = nn.Dropout2d(self.dropout_rate)
        self.drop_l_dec3 = nn.Dropout2d(self.dropout_rate)
        self.drop_final = nn.Dropout2d(self.dropout_rate)

        # CBAM-style gate re-weights global encoder bottleneck features spatially
        self.attention_gate = SpatialAttention(kernel_size=7)
    ...
    def _forward_impl(self, x):
        ...
        attn_weights = self.attention_gate(x2)
        x2 = x2 * attn_weights

        x = F.leaky_relu(F.interpolate(self.decoder4(x2), scale_factor=(2,2), mode='bilinear'), negative_slope=0.01)
        x = torch.add(x, x1)
        x = F.leaky_relu(F.interpolate(self.decoder5(x), scale_factor=(2,2), mode='bilinear'), negative_slope=0.01)
        ...
        for i in range(4):
            for j in range(4):
                patches.append(xin[:, :, 64*i:64*(i+1), 64*j:64*(j+1)])

        x_patches = torch.cat(patches, dim=0)

        # depthwise convs + BN under float16 can produce NaN; run local branch in float32
        with torch.amp.autocast(device_type='cuda', enabled=False):
            x_patches_f32 = x_patches.float()

            x_p = self.conv1_p(x_patches_f32)
            x_p = self.bn1_p(x_p)
            x_p = F.relu(x_p)
            ...
            if self.training:
                x4_p = self.drop_l_enc4(x4_p)  # bottleneck dropout

            x_p = self.decoder1_p(x4_p)
            x_p = torch.add(x_p, x4_p)
            x_p = F.leaky_relu(F.interpolate(x_p, scale_factor=(2,2), mode='bilinear'), negative_slope=0.01)
            x_p = self.decoder2_p(x_p)
            x_p = torch.add(x_p, x3_p)
            if self.training:
                x_p = self.drop_l_dec2(x_p)
            x_p = F.leaky_relu(F.interpolate(x_p, scale_factor=(2,2), mode='bilinear'), negative_slope=0.01)
            x_p = self.decoder3_p(x_p)
            x_p = torch.add(x_p, x2_p)
            if self.training:
                x_p = self.drop_l_dec3(x_p)
            x_p = F.leaky_relu(F.interpolate(x_p, scale_factor=(2,2), mode='bilinear'), negative_slope=0.01)
            x_p = self.decoder4_p(x_p)
            x_p = torch.add(x_p, x1_p)
            x_p = F.leaky_relu(F.interpolate(x_p, scale_factor=(2,2), mode='bilinear'), negative_slope=0.01)
            x_p = self.decoder5_p(x_p)

            # Cross-patch communication before fusion
            x_loc = torch.zeros(
                x_patches_f32.shape[0] // 16,
                x_p.shape[1],
                xin.shape[2],
                xin.shape[3],
                device=x_p.device,
            ...
            for idx in range(16):
                i, j = divmod(idx, 4)
                x_loc[:, :, 64*i:64*(i+1), 64*j:64*(j+1)] = x_p[idx*B:(idx+1)*B]

            x_loc = self.cross_patch_comm(x_loc)

        x_cat   = torch.cat([x, x_loc.to(x.dtype)], dim=1)
        x_fused = self.fusion_block(x_cat)
        x       = F.leaky_relu(self.decoderf(x_fused), negative_slope=0.01)

        if self.training:
            x = self.drop_final(x)
        x = self.adjust_p(F.leaky_relu(x, negative_slope=0.01))
